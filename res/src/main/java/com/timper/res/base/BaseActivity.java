...
public class BaseActivity<V extends AppViewModel, T extends ViewDataBinding> extends AppActivity<V, T> {
  ...
  @SuppressWarnings("unused") protected void replaceFragment(int frameLayoutId, Fragment fragment) {
    if (fragment != null) {
      FragmentTransaction transaction = getSupportFragmentManager().beginTransaction();
  ...
  @SuppressLint("PrivateApi") private void setMIUIStatusBarDarkMode() {
    if (isMiUi) {
  ...
  static {
    try {
      ...
      isMiUi = !(version.compareTo("V9") >= 0 && Build.VERSION.SDK_INT >= 23)
          && version.compareTo("V6") >= 0
   ...
   */
  @SuppressWarnings("JavaReflectionMemberAccess") private static boolean setMeizuDarkMode(Window window) {
  ...
  @SuppressLint("InlinedApi") private int getStatusBarLightMode() {
    int result = 0;
  ...
  @SuppressLint("InlinedApi") protected void setStatusBarDarkMode() {
    ...
    if (type == 1) {
      ...
      if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP && isSetStatusBarColor()) {
        ...
        window.getDecorView().setSystemUiVisibility(View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN | View.SYSTEM_UI_FLAG_LAYOUT_STABLE);
        window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
        window.setStatusBarColor(Color.parseColor("#CFCFCF"));
      }
    }
  }
}
