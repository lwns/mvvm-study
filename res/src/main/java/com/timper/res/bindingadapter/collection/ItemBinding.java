      ...
      if (onItemBind.getOnItemClick(item) != null) {
        binding.getRoot().setOnClickListener(new View.OnClickListener() {
          @Override public void onClick(View v) {
            onItemBind.getOnItemClick(item).onItemClick(v, binding, position);
          }
        });
      }
      if (onItemBind.getOnItemChildClick(item) != null && onItemBind.getOnItemChildClick(item).size() > 0) {
        for (final Map.Entry<Integer, OnItemChildClickListener> entry : onItemBind.getOnItemChildClick(item).entrySet()) {
          View view = binding.getRoot().findViewById(entry.getKey());
          if (view != null) {
            view.setOnClickListener(new View.OnClickListener() {
              @Override public void onClick(View v) {
                entry.getValue().onItemChildClick(v, binding, position);
              }
            });
          }
        }
