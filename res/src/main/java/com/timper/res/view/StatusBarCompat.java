...
/**
 * Created by ning.li.o on 2017/12/4.
 */
...
public class StatusBarCompat {
  ...
  static {
    IStatusBar defaultStatus = new IStatusBar() {
      @Override public void setStatusBarColor(Window window, int color) {
      }
    };
  ...
  static class StatusBarMImpl implements IStatusBar {
    ...
    @TargetApi(Build.VERSION_CODES.M) public void setStatusBarColor(Window window, int color) {
  ...
  static class StatusBarLollipopImpl implements IStatusBar {
    ...
    @TargetApi(Build.VERSION_CODES.LOLLIPOP) public void setStatusBarColor(Window window, int color) {
      //取消设置透明状态栏,使 ContentView 内容不再覆盖状态栏
      window.clearFlags(
          WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS | WindowManager.LayoutParams.FLAG_TRANSLUCENT_NAVIGATION);
      window.getDecorView().setSystemUiVisibility(View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN | View.SYSTEM_UI_FLAG_LAYOUT_STABLE);
  ...
  static class StatusBarKitkatImpl implements IStatusBar {
    ...
    @TargetApi(Build.VERSION_CODES.KITKAT) public void setStatusBarColor(Window window, int color) {
